...
Byheart records external decisions/actions/receipts instead of model-private reasoning. A compiler turns a successful trace into a parameterized capability. Replay is a separate engine with no model decision loop: it renders invocation parameters, checks policy and preconditions, performs recorded actions, verifies effects/checkpoints, handles declared outcomes/recoveries, extracts outputs, and returns a typed result.
...
Successful discovery traces can be parameterized while compiling. For example, a trace learned with market `ASH-17` and quantity `25` can emit `{{input.market}}` and `{{input.quantity}}`, then replay with different values. The take-home demo also declares typed outputs for staged-order reference, confirmed market, accepted quantity, and total credits. These are extracted from accessible status targets on the final UI, so replay returns application-observed values rather than merely echoing inputs.
...
The Playwright adapter searches the top page and every iframe and requires semantic targets to resolve exactly once. Role + accessible name and labels are preferred; selectors and points remain explicit fallbacks. Waits are bounded. Final checkpoints and output extraction are independent from action delivery.
...
The repository already includes follow-on adapter prototypes for a screenshot/mouse/keyboard remote session and Preflight's closed Starsector request/receipt protocol. They demonstrate that the core contract extends beyond the DOM; they are not dependencies of the browser submission.
